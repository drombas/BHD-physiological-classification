function Y = get_time_features(t, S, found_peaks)
...
for n=1:n_s
    ...
    mins = found_peaks(n).min;
    maxs = found_peaks(n).max;
    ...
    % Compute features
    Y(n).ad_up = mean(ad_up);
    Y(n).ad_down = mean(ad_down);
    aux = 100*(time_up-time_total)./time_total;
    Y(n).time_up_rel = mean(aux);
    Y(n).NT = 100*sum(aux < 50)/length(aux);
end